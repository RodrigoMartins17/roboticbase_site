...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    <h4 class="mb-0 fw-bold" style="color: #1e293b;"><i class="fas fa-door-open text-primary me-2"></i>Requisições de Salas</h4>
    
    <a href="<?php echo BASE_URL; ?>admin/reqSalaCreate" class="btn-clean-primary">
        <i class="fas fa-plus me-2"></i> Nova Requisição
    </a>
</div>

<?php
$salaLabel = function ($r) {
    $bloco = $r['bloco'] ?? '';
    $andar = $r['andar'] ?? '';
    $numero = str_pad($r['sala_numero'] ?? '', 2, '0', STR_PAD_LEFT);
    return trim($bloco . $andar . '.' . $numero) ?: '—';
};

$renderTableRows = function ($items) use ($salaLabel) {
    if (empty($items)) {
    ...
    foreach ($items as $r) {
        ...
        $inicio = (!empty($r['data_inicio']) && ($ts = strtotime($r['data_inicio'])) !== false) ? date('d/m/Y H:i', $ts) : '—';
        $fim = (!empty($r['data_fim']) && ($ts = strtotime($r['data_fim'])) !== false) ? date('d/m/Y H:i', $ts) : '—';
        ?>
        <tr>
            <td class="ps-4"><?php echo $id; ?></td>
            <td>
                <div class="d-flex align-items-center gap-3">
                    <div style="width: 36px; height: 36px; border-radius: 8px; background-color: #f1f5f9; display: flex; align-items: center; justify-content: center; color: #94a3b8;"><i class="fas fa-map-marker-alt"></i></div>
                    <div>
                        <strong><?php echo htmlspecialchars($salaLabel($r)); ?></strong><br>
            ...
            <td class="text-end pe-4 text-nowrap">
                <a href="<?php echo BASE_URL; ?>admin/reqSalaView/<?php echo $id; ?>" class="action-icon-btn action-view" title="Ver Detalhes"><i class="fas fa-eye"></i></a>
                <?php if ($st === 'PENDENTE'): ?>
                    <a href="<?php echo BASE_URL; ?>admin/reqSalaAceitar/<?php echo $id; ?>" class="action-icon-btn" style="background-color: #10b981;" onclick="return confirm('Aceitar requisição?');"><i class="fas fa-check"></i></a>
                    <a href="<?php echo BASE_URL; ?>admin/reqSalaRejeitar/<?php echo $id; ?>" class="action-icon-btn action-delete" onclick="return confirm('Rejeitar requisição?');"><i class="fas fa-times"></i></a>
                <?php endif; ?>
                <?php if ($st === 'ACEITE'): ?>
                    <a href="<?php echo BASE_URL; ?>admin/reqSalaEntregar/<?php echo $id; ?>" class="action-icon-btn" style="background-color: #0ea5e9;" title="Marcar como entregue (check-in)" onclick="return confirm('Marcar sala como entregue?');"><i class="fas fa-door-open"></i></a>
                <?php endif; ?>
                <?php if ($st === 'EM_USO'): ?>
                    <a href="<?php echo BASE_URL; ?>admin/reqSalaDevolver/<?php echo $id; ?>" class="action-icon-btn" style="background-color: #16a34a;" title="Marcar como devolvida ao clube"><i class="fas fa-undo"></i></a>
                <?php endif; ?>
                <a href="<?php echo BASE_URL; ?>admin/reqSalaEdit/<?php echo $id; ?>" class="action-icon-btn action-edit" title="Editar"><i class="fas fa-pen"></i></a>
                <a href="<?php echo BASE_URL; ?>admin/reqSalaDelete/<?php echo $id; ?>" class="action-icon-btn action-delete" title="Apagar"><i class="fas fa-trash-alt"></i></a>
            </td>
        </tr>
        <?php
    }
...
<div class="clean-card mb-4">
    <div class="px-4 py-3 border-bottom" style="background-color: #f8fafc;">
        <h6 class="mb-0 fw-bold" style="color: #334155;"><i class="fas fa-door-open text-success me-2"></i> Aceites / Ativas <span class="badge bg-success ms-2"><?php echo count($aceites ?? []); ?></span></h6>
    </div>
    <div class="table-responsive">
        <table class="table table-borderless table-clean mb-0">
            <thead>
