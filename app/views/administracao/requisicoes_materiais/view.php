...
// Função auxiliar para cores do estado
    ...
    <div class="col-md-8">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
            ...
            <div class="d-flex flex-wrap gap-2">
                <a href="<?php echo BASE_URL; ?>admin/reqMaterialEdit/<?php echo $id; ?>" class="btn-clean-outline" style="background-color: #fffbeb; color: #d97706; border-color: #fde68a;">
                    <i class="fas fa-pen me-2"></i>Editar
                </a>
              <a href="<?php echo BASE_URL; ?>admin/reqMaterialDelete/<?php echo $id;?>" class="btn-clean-outline" style="background-color: #fef2f2; color: #dc2626; border-color: #fecaca;" title="Apagar">
                <i class="fas fa-trash-alt me-2"></i>Eliminar</a>
                <a href="<?php echo BASE_URL; ?>admin/requisicoesMateriais" class="btn-clean-outline">
                    Voltar
                </a>
            </div>
        </div>
        ...
        <div class="clean-card p-4 p-md-5 mb-5">
            ...
            <div class="mb-2">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-6">
                        <div class="info-box">
                            <h6 class="info-label">Material Solicitado</h6>
                            <span style="font-weight: 700; color: #1e293b; font-size: 1.05rem;"><?php echo htmlspecialchars($r['material_designacao'] ?? '—'); ?></span>
                            <span style="color: #2563eb; font-family: monospace; font-weight: 600; font-size: 0.9rem; margin-top: 2px;">Ref: <?php echo htmlspecialchars($r['num_referencia'] ?? '—'); ?></span>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="info-box">
                            <h6 class="info-label">Data de Levantamento</h6>
                            <span style="font-weight: 600; color: #334155; font-size: 1.05rem;">
                                <?php echo !empty($r['data_levantamento']) && ($ts = strtotime($r['data_levantamento'])) !== false ? date('d/m/Y \à\s H:i', $ts) : '—'; ?>
                            </span>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="info-box">
                            <h6 class="info-label">Data de Devolução</h6>
                            <span style="font-weight: 600; color: #334155; font-size: 1.05rem;">
                                <?php echo !empty($r['data_devolucao']) && ($ts = strtotime($r['data_devolucao'])) !== false ? date('d/m/Y \à\s H:i', $ts) : '—'; ?>
                            </span>
                            <?php if (!empty($r['estado_devolucao'])): ?>
                                <span style="display: inline-block; background-color: #f1f5f9; color: #475569; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 700; border: 1px solid #cbd5e1; margin-top: 6px; width: fit-content;">
                                    <?php echo htmlspecialchars($r['estado_devolucao']); ?>
                                </span>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <?php if (in_array($estado, ['PENDENTE', 'ACEITE', 'EM_USO'])): ?>
                <div class="d-flex flex-wrap justify-content-center gap-3 pt-4 mt-4 border-top" style="border-color: #e2e8f0 !important;">
                    ...
                    <?php if ($estado === 'PENDENTE'): ?>
                        <a href="<?php echo BASE_URL; ?>admin/reqMaterialAceitar/<?php echo $id; ?>" class="btn-workflow" style="background-color: #10b981; color: white; border: 1px solid #059669;" onclick="return confirm('Confirmar aprovação deste pedido?');">
                            <i class="fas fa-check-circle me-2"></i>Aprovar Pedido
                        </a>
                        <a href="<?php echo BASE_URL; ?>admin/reqMaterialRejeitar/<?php echo $id; ?>" class="btn-workflow" style="background-color: #ef4444; color: white; border: 1px solid #dc2626;" onclick="return confirm('Rejeitar esta requisição?');">
                            <i class="fas fa-times-circle me-2"></i>Rejeitar Pedido
                        </a>
                    <?php elseif ($estado === 'ACEITE'): ?>
                        <a href="<?php echo BASE_URL; ?>admin/reqMaterialEntregar/<?php echo $id; ?>" class="btn-workflow" style="background-color: #0ea5e9; color: white; border: 1px solid #0284c7;" onclick="return confirm('Marcar como entregue ao aluno?');">
                            <i class="fas fa-box-open me-2"></i>Marcar entregue ao aluno
                        </a>
                    <?php elseif ($estado === 'EM_USO'): ?>
                        <a href="<?php echo BASE_URL; ?>admin/reqMaterialDevolver/<?php echo $id; ?>" class="btn-workflow" style="background-color: #16a34a; color: white; border: 1px solid #15803d;">
